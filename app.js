...
function initQuiz(){currentSheet=sheetSelect.value;mastered.clear();pool=[...vocabSets[currentSheet]];startRound();setupDiv.style.display='none';quizDiv.style.display='block';}
function startRound(){
 ...
 questions=batch.map(e=>{const opts=genOpts(e.definition);return{e,opts,correctIndex:opts.indexOf(e.definition),selected:null,wrong:false};});
 currentIndex=0;wrongPool=[];showQ();
}
function showQ(){
 const q=questions[currentIndex];
 progressDiv.textContent=`Question ${currentIndex+1}/${questions.length}  (Mastered ${mastered.size}/${pool.length})`;
 questionDiv.textContent=q.e.word;
